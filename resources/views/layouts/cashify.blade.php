  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .menu a.is-active::before{
      ...
      border:1px solid rgba(109,92,255,.75);
      pointer-events:none;
    }
    .menu a.is-active .icon{
      color:#e5e7eb;
    }
    .menu a.is-active .text{
      font-weight:600;
      color:#f9fafb;
    }
    ...
    /* Welcome kiri topbar (hanya dipakai di dashboard via Blade) */
    .welcome-top{
      display:block;
      padding:0;
      border:none;
    ...
    .dropdown.show .dd-item{
      animation: ddIn .22s ease both;
    }
    @keyframes ddIn{
    ...
    .modal.hiding{
      animation:modalOut .22s ease both;
    }
    .modal-head{
      display:flex;
      align-items:center;
    ...
    @keyframes modalIn{
      ...
      60%{transform:translateY(0) scale(1.01);opacity:1;filter:blur(.5px)}
      100%{transform:translateY(0) scale(1);opacity:1;filter:blur(0)}
    }
    @keyframes modalOut{
      0%{transform:translateY(0) scale(1);opacity:1;filter:blur(0)}
      100%{transform:translateY(8px) scale(.985);opacity:0;filter:blur(2px)}
    }
  ...
  <aside class="sidebar">
    ...
    <nav class="menu">
      ...
      <a href="{{ route('produk.index') }}"
        ...
        <svg class="icon" viewBox="0 0 24 24" fill="none">
          <path d="M6 7l1-3h10l1 3M6 7h12l-1 10H7L6 7zm3 4h6"
                stroke="currentColor" stroke-width="1.5"/>
        </svg>
        <span class="text">Produk</span>
      </a>
      <a href="{{ route('transaksi.index') }}"
         class="{{ request()->routeIs('transaksi.*') ? 'is-active' : '' }}">
        <svg class="icon" viewBox="0 0 24 24" fill="none">
          <path d="M4 7h16M7 11h10M9 15h6M6 19h12"
                stroke="currentColor" stroke-width="1.5"/>
        </svg>
...
<div id="logoutBackdrop" class="modal-backdrop" aria-hidden="true">
  <div id="logoutModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="logoutTitle" aria-describedby="logoutDesc" tabindex="-1">
    <div class="modal-head">
      <div class="modal-title" id="logoutTitle">Konfirmasi Logout</div>
    </div>
